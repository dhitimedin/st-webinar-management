<?php
/**
 * File containing the class ST_Webinar_Management_Dependency_Checker.
 *
...
/**
 * Handles checking for ST_Webinar_Management's dependencies.
 *
 * @since 1.0.0
 */
class ST_Webinar_Management_Dependency_Checker {
	const MINIMUM_PHP_VERSION = '7.4.0';
	const MINIMUM_WP_VERSION  = '5.8.0';

	/**
	 * Check if ST_Webinar_Management's dependencies have been met.
	 *
	 * @return bool True if we should continue to load the plugin.
	 */
	public static function check_dependencies() {
		if ( ! self::check_php() ) {
			add_action( 'admin_notices', array( 'ST_Webinar_Management_Dependency_Checker', 'add_php_notice' ) );
		...
		if ( ! self::check_wp() ) {
			add_action( 'admin_notices', array( 'ST_Webinar_Management_Dependency_Checker', 'add_wp_notice' ) );
			add_filter( 'plugin_action_links_' . ST_WEBINAR_MANAGEMENT_PLUGIN_BASENAME, array( 'ST_Webinar_Management_Dependency_Checker', 'wp_version_plugin_action_notice' ) );
		}
	...
	public static function add_php_notice() {
		...
		// translators: %1$s is version of PHP that Webinar Management requires; %2$s is the version of PHP WordPress is running on.
		$message = sprintf( __( '<strong>Webinar Management</strong> requires a minimum PHP version of %1$s, but you are running %2$s.', 'st-webinar-management' ), self::MINIMUM_PHP_VERSION, phpversion() );
		...
		printf(
			...
			esc_html__( 'Learn more about updating PHP', 'st-webinar-management' ),
			/* translators: accessibility text */
			esc_html__( '(opens in a new tab)', 'st-webinar-management' )
		);
		echo '</p></div>';
	}
	...
	public static function deactivate_self() {
		deactivate_plugins( ST_WEBINAR_MANAGEMENT_PLUGIN_BASENAME );
	}
	...
	public static function add_wp_notice() {
		...
		if ( current_user_can( 'update_core' ) ) {
			...
			$update_action_link = ' ' . sprintf( __( 'Please <a href="%s">update WordPress</a> to avoid issues.', 'st-webinar-management' ), esc_url( self_admin_url( 'update-core.php' ) ) );
		}

		echo '<div class="error">';
		echo '<p>' . wp_kses_post( __( '<strong>Webinar Management</strong> requires a more recent version of WordPress.', 'st-webinar-management' ) . $update_action_link ) . '</p>';
		echo '</div>';
	}
	...
	public static function wp_version_plugin_action_notice( $actions ) {
		if ( ! current_user_can( 'update_core' ) ) {
			$actions[] = '<strong style="color: red">' . esc_html__( 'WordPress Update Required', 'st-webinar-management' ) . '</strong>';
		} else {
			$actions[] = '<a href="' . esc_url( self_admin_url( 'update-core.php' ) ) . '" style="color: red">' . esc_html__( 'WordPress Update Required', 'st-webinar-management' ) . '</a>';
		}
		return $actions;
	}
